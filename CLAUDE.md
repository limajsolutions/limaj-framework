...
| [Limaj.Framework.Functions](packages/Limaj.Framework.Functions/) | `Abstractions` + `Microsoft.AspNetCore.App` |
...
- **Result / Error flow** — [Result.cs](packages/Limaj.Framework.Abstractions/src/Common/Result.cs) and [Error.cs](packages/Limaj.Framework.Abstractions/src/Common/Error.cs). Application code returns `Result`/`Result<T>`; HTTP layer maps via [ResultExtensions.ToHttpResult](packages/Limaj.Framework.Functions/src/Http/ResultExtensions.cs) which translates `ErrorType` → HTTP status (Validation→400, NotFound→404, Conflict→409, Forbidden→403, Unauthorized→401, TooManyRequests→429, Unexpected→500).
- **Exception bridge** — Domain code throws `DomainValidationException` / `NotFoundException` / `ConflictException` from [Errors/](packages/Limaj.Framework.Abstractions/src/Errors/); [FunctionRunner.RunAsync](packages/Limaj.Framework.Functions/src/Http/FunctionRunner.cs) + [ExceptionExtensions](packages/Limaj.Framework.Functions/src/Http/ExceptionExtensions.cs) wrap handlers and convert these to structured HTTP responses. New endpoints should be wrapped in `FunctionRunner.RunAsync` rather than hand-written try/catch.
